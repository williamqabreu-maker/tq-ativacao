...
async function sendInitiateCheckoutEvent({ accessToken, clientName, clientEmail, clientPhone, planName, transKey, value, currency, testEventCode }) {
  const eventTime = Math.floor(Date.now() / 1000);
  const eventId = `checkout_${transKey || Date.now()}`;
  const { fn, ln } = normalizeName(clientName);
  const phoneNorm = normalizePhone(clientPhone);
  ...
  try {
    ...
    const eventsReceived = res.data?.events_received || 0;
    console.log('[CAPI] InitiateCheckout enviado — event_id:', eventId, '| events_received:', eventsReceived);
    return { ok: true, events_received: eventsReceived, event_id: eventId, readable_payload: readablePayload };
  } catch (e) {
    const errMsg = e.response?.data?.error?.message || e.message;
    console.error('[CAPI] InitiateCheckout Erro:', errMsg);
    throw new Error(`CAPI InitiateCheckout: ${errMsg}`);
  }
}
